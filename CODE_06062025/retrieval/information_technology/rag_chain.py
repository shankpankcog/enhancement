...
@dataclass
class CustomInputs():
    filters: Dict[str, str] = field(default_factory=lambda: {"doc_control_name": ['Information Technology']})
...
def create_configurable_with_filters(input: Dict) -> Dict:
    ...
    # else:
    #     filters = {}
    filters={"doc_control_name": ["Information Technology"]}
    configurable = {
        "configurable": {
            "search_kwargs": {
                "k": 8,
...
vector_search_as_retriever = DatabricksVectorSearch(
    ...
    ).as_retriever(search_kwargs={"k": 8, "query_type": "HYBRID", "fetch_k": 40})
